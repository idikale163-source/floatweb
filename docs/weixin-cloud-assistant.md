...
- **冷启动**：Bot 长时间无人轮询时，微信服务器会把它标记为离线（微信里显示
  ...
  冷启动只有约 20ms）。
- **提示词等价性**：运行包里的轻量模板（v2）把「深度注入」单独切成段下发——
  世界书 `position=4` 与预设 `injection_position≠0` 的条目，助手会按当前微信消息
  条数重新定位到「倒数第 N 条」之前，而不是沿用同步那一刻算好的位置；组装完还会
  合并相邻同 role 消息，让模型看到的轮次粒度与小手机一致。老运行包（v1）没有这些
  字段时助手自动退回旧行为，**升级后需要在小手机里重新同步一次运行包才生效**。
  同步时会顺带把新的 `assistant-core.mjs` 推到桶里，云函数下一轮自动加载，无需重新部署。
