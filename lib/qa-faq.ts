...
两个常见原因：① 模型单次回复有最大输出 token 上限（中转站往往只有几千），大 APP 的完整 HTML 一次写不完——小坊支持分段写入：APP 用「暂存应用文件」分多轮追加、最后「安装暂存应用」组包；大游戏用「保存游戏草稿」的 gameHtmlAppend 分轮追加、写完「安装本机游戏」fromDraft 上架；剧场超长开场同理用 openingHtmlAppend。任何输出上限都不受影响；也可到「设置 → 预设」调大最大输出 token。② 屏幕上出现 <|open|>call tool= 之类的乱码：说明该 API 渠道不支持原生工具调用（没把模型的内部工具格式翻译成标准结构），到「设置 → API 设置」把该配置的「原生工具」开关关掉，工坊会改走文本协议，任何模型都能用；或更换正规渠道。
