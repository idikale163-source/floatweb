...
### Q: 小坊回复到一半突然停了是怎么回事？
几种常见情况：① 单轮对话的工具调用轮数用尽（默认 8 轮，工坊配置可调），提示后回复「继续」即可接着做；② 模型输出长度上限把内容截断（写大型 APP 时最常见）——小坊会自动续接，不用人管；若连自动续写轮数也用完会提示回复「继续」。在「工坊配置 → 单次最大输出 token」设置护栏值后更稳：写超被安全截断后自动分段接力；③ 输出中途停顿几分钟后一口气全出：流式连接停滞（常见于中转站开了响应缓冲），停滞超 90 秒会自动中断并改用整段获取，消息里会显示切换说明——需等完整生成后一次性显示，属正常降级；④ 正在生成大段工具调用（如整个 APP 的代码）时会显示「正在编写工具调用…」，不是卡死。
