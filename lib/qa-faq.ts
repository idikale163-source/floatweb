...
### Q: 怎么关闭角色主动消息？
「设置 → 预设」里打开正在使用的预设，关闭条目「▸ 固定时间主动消息」。相关条目还有「▸ 稍后主动联系」「▸ 冷场重连」「▸ 追发提示」，按需一并关闭。
...
### Q: 生图报错的常见原因？
生图 key 未配置或失效、额度不足、生图代理（NEXT_PUBLIC_IMAGE_GEN_PROXY_URL 或应用内生图设置）不可达、所选模型不支持当前参数。先检查生图 API 配置与余额，再用工坊「最近报错」看具体报错内容。
`;
